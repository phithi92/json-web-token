...
class JwtValidator
{
     ...
     * @param int         $clockSkew              Allowed clock skew (in seconds) when validating time-based claims
     *                                            like "exp", "nbf", and "iat". Helps tolerate minor time drift.
     * @param array       $expectedPrivateClaims  Optional associative array of expected private claims.
    ...
    private function isValidPrivateClaims(JwtPayload $payload): bool
    {
        foreach ($this->expectedPrivateClaims as $key => $expectedValue) {
            ...
            $actualValue = $payload->getField($key);
     ...
     * @throws InvalidPrivateClaimException If any claim is missing or does not match.
     */
    private function assertValidPrivateClaims(JwtPayload $payload): void
    {
        foreach ($this->expectedPrivateClaims as $key => $expectedValue) {
            $exist = $payload->hasField($key);

            // If the claim is missing and we're checking for existence or specific value
            if (!$exist) {
            ...
            // If a specific value is expected and doesn't match, throw exception
            if ($expectedValue !== null && $actualValue !== $expectedValue) {
                throw new InvalidPrivateClaimException($key, $expectedValue);
            }
        }
    }
     ...
     *
     * @param JwtPayload $payload The decoded payload.
     * @return bool True if all checks pass.
     */
    public function isValid(JwtPayload $payload): bool
    {
        return $this->isValidPrivateClaims($payload)
            && $this->isNotExpired($payload)
            && $this->isNotBeforeValid($payload)
            && $this->isIssuedAtValid($payload)
            && $this->hasValidIssuer($payload)
            && $this->hasValidAudience($payload);
    }
    ...
    public function isNotExpired(JwtPayload $payload): bool
    {
        ...
        return ($exp + $this->clockSkew) > time();
    }
     ...
     */
    public function hasValidIssuer(JwtPayload $payload): bool
    {
        if ($this->expectedIssuer === null) {
            return true;
        }

        return $payload->getIssuer() === $this->expectedIssuer;
    }
     ...
     */
    public function hasValidAudience(JwtPayload $payload): bool
    {
        ...
        if (is_array($aud)) {
            return in_array($this->expectedAudience, $aud, true);
        }

        return $aud === $this->expectedAudience;
    }

    /**
     * Validates the given encrypted JWT bundle.
     *
     * @param EncryptedJwtInterface $bundle The encrypted JWT to validate
     *
    ...
    {
        $this->assertValidPrivateClaims($payload);
        $this->assertNotExpired($payload);
        $this->assertNotBeforeValid($payload);
    ...
    private function assertNotExpired(JwtPayload $payload): void
    {
        ...
        } elseif (($exp + $this->clockSkew) <= time()) {
    ...
    private function assertValidIssuer(JwtPayload $payload): void
    {
        if ($this->expectedIssuer !== null && $payload->getIssuer() !== $this->expectedIssuer) {
            throw new InvalidIssuerException($this->expectedIssuer, $payload->getIssuer());
        }
    }
